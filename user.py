...
import re, json, requests, asyncio, os, time, datetime
...
@client.on(events.NewMessage(chats=[-1001320212725, my_chat_id], pattern=r'.*dou|.*gua |.*豆'))
async def shopbean(event):
    cookies = myck(_ConfigFile)
    message = event.message.text
    url = re.findall(re.compile(r"[(](https://api\.m\.jd\.com.*?)[)]", re.S), message)
...
##监控光头哥群组队1
@client.on(events.NewMessage(chats=[-1001320212725, my_chat_id], pattern=r"^export jd_zdjr_.*=\".*\""))
async def myexport(event):
    try:
        ...
        await cmd('jtask /jd/own/smiek_jd_zdjr.js now && jtask /jd/own/smiek_jd_zdjr.js now')
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))

##监控光头哥群组队2
@client.on(events.NewMessage(chats=[-1001320212725, my_chat_id], pattern=r"^export OPEN_CARD_.*=\".*\""))
async def myexport(event):
    try:
        messages = event.message.text.split("\n")
        msg = await jdbot.send_message(chat_id, "监控到新的入会变量，准备自动替换")
        for message in messages:
            ...
            with open(f"{_ConfigDir}/config.sh", 'w', encoding='utf-8') as f3:
                f3.write(configs)
        await jdbot.edit_message(msg, end)
        await cmd('jtask /jd/own/jd_open_card_by_shopid.js now && jtask /jd/own/jd_open_card_by_shopid.js now') #V4自行改成jtask + 脚本绝对路径 now
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))
        
##监控光头哥群组队2
@client.on(events.NewMessage(chats=[-1001320212725, my_chat_id], pattern=r"^export jd_joinTeam_.*=\".*\""))
async def myexport(event):
    try:
        messages = event.message.text.split("\n")
        msg = await jdbot.send_message(chat_id, "监控到新的入会变量，准备自动替换")
        for message in messages:
            ...
            with open(f"{_ConfigDir}/config.sh", 'w', encoding='utf-8') as f3:
                f3.write(configs)
        await jdbot.edit_message(msg, end)
        await cmd('jtask /jd/own/gua_joinTeam.js now && jtask /jd/own/gua_joinTeam.js now')
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))

##监控光头哥群关注有礼
@client.on(events.NewMessage(chats=[-1001320212725, my_chat_id], pattern=r"^export FAV_SHOP_ID.*=\".*\""))
async def myexport(event):
    try:
        messages = event.message.text.split("\n")
        msg = await jdbot.send_message(chat_id, "监控到新的入会变量，准备自动替换")
        for message in messages:
            ...
            with open(f"{_ConfigDir}/config.sh", 'w', encoding='utf-8') as f3:
                f3.write(configs)
        await jdbot.edit_message(msg, end)
        await cmd('jtask /jd/own/jd_fav_shop_gift.js now && jtask /jd/own/jd_fav_shop_gift.js now')
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))

##监控光头哥群关注有礼2
@client.on(events.NewMessage(chats=[-1001320212725, my_chat_id], pattern=r"^export ISV_SHOP_ID=\".*\""))
async def myexport(event):
    try:
        messages = event.message.text.split("\n")
        msg = await jdbot.send_message(chat_id, "监控到新的入会变量，准备自动替换")
        for message in messages:
            ...
            with open(f"{_ConfigDir}/config.sh", 'w', encoding='utf-8') as f3:
                f3.write(configs)
        await jdbot.edit_message(msg, end)
        await cmd('jtask /jd/own/jd_follow_wxshop_gift_lof.js now && jtask /jd/own/jd_follow_wxshop_gift_lof.js now')
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))

##监控光头哥群店铺签到
@client.on(events.NewMessage(chats=[-1001320212725, my_chat_id], pattern=r"^export MyShopToken=\".*\""))
async def myexport(event):
    try:
        messages = event.message.text.split("\n")
        msg = await jdbot.send_message(chat_id, "监控到新的入会变量，准备自动替换")
        for message in messages:
            ...
            with open(f"{_ConfigDir}/config.sh", 'w', encoding='utf-8') as f3:
                f3.write(configs)
        await jdbot.edit_message(msg, end)
        await cmd('jtask /jd/own/jd_shop_sign.js now && jtask /jd/own/jd_shop_sign.js now')
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))
    ...
    try:
        ...
        for RRA in RRAs:
            ...
            if V4:
                ...
                cmdtext = f'/cmd task /ql/jredrain.sh {RRA}'
